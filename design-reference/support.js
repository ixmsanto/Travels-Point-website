...
return void 0;
    ...
    if (expr[0] === "!") {
return !resolve(vals, expr.slice(1));
}
...
props.children = kids.map((b, j) => b(vals, ctx, j));
  ...
  function walkXImport(el, host) {
    ...
    if (urls.length && !urlBindable) {
      ...
      for (const u of urls) {
prev = host.loadExternal(kindOf(u), u, prev);
}
    }
...
return;
    ...
    window.addEventListener("message", (e) => {
      if (!designDocMode || (e.data && e.data.type) !== "__dc_probe") {
return;
}
